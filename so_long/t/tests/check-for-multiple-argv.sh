...
EXTRA2="maps/valid/map3.ber"
  VAL1=$(valgrind --leak-check=full --track-origins=yes --quiet --tool=memcheck ./../so_long $MAP $EXTRA1 | grep "Error" > check)
  VAL2=$(valgrind --leak-check=full --track-origins=yes --quiet --tool=memcheck ./../so_long $MAP $EXTRA1 $EXTRA2 | grep "Error" > check)
